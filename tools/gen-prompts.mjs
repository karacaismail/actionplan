...
const DIM = {
  featureDefs: {
    tr: "Özellik Tanımları",
    kapsa: "net işlevsel kapsam, girdi/çıktı sözleşmesi, durum makinesi, hata yolları",
  },
  security: {
    tr: "Güvenlik Önlemleri",
    kapsa:
      "tenant izolasyonu (PostgreSQL RLS), PII sınıflandırma, rol bazlı erişim (least-privilege), değişmez audit kanıtı",
  },
  codeOptimization: {
    tr: "Kod Optimizasyonu",
    kapsa: "tip güvenliği, ölü kod elemesi, modülerlik, kod bölme, döngüsel karmaşıklık sınırı",
  },
  securityOptimization: {
    tr: "Güvenlik Optimizasyonu",
    kapsa:
      "secret rotasyonu, sıkı CSP, rate-limit, en az ayrıcalık sertleştirme, bağımlılık taraması",
  },
  performance: {
  ...
  mobileApps: {
    tr: "Mobil Uygulama Uyumu",
    kapsa:
      "iOS/Android (PWA veya Capacitor), Chrome extension yüzeyi, dokunma hedefi ≥44px, offline davranışı",
  },
  wcag: {
    tr: "WCAG 2.2 AAA",
  ...
  deployment: {
    tr: "Dağıtım",
    kapsa:
      "Docker Swarm servisi, Kubernetes (HPA + liveness/readiness probe + kaynak limiti), WordPress sınıfı shared hosting kısıtı/uyumu",
  },
  eca: {
    tr: "ECA Kuralları",
    kapsa:
      "backend/engine tarafında çalışan ruleset (UI yalnız güvenli yönetim yüzeyi, serbest JS/SQL/shell yok). AI app/module üretim-güncelleme denemesinde deny; ArcheType prod update için geçmiş veri koruma, snapshot, rollback, compatibility, append-only/expand-contract; döngü kırıcı maks 6, idempotency, tenant isolation, action allowlist, step-up; ruleset yetki katmanları (system kilitli / platform owner / tenant yalnız güvenli parametre)",
  },
  aiAgents: {
    tr: "AI Ajan Davranışı",
    kapsa:
      "AI yalnız ArcheType taslağı/prod-update önerisi üretebilir; app/module üretemez/güncelleyemez; ruleset override edemez; doğrudan prod write yapamaz. ArcheType çok-parçalı sözleşme (fields/relations/permissions/ReBAC-ABAC/lifecycle/surfaces/workflows/migration/ai-policy). Admin akışı: prompt→draft→validation→diff→data-impact→dry-run→preview→approval→apply. AI doğrulama döngüsü: 1-3 otomatik, 3/6 admin'e çok-seçenekli soru, 9 tanı+dur. sub_prompt güvenilmez; kill-switch; step-up; PII redaksiyon",
  },
  testing: {
    tr: "Testler & QA",
    kapsa:
      "unit + e2e + kullanıcı yolculuğu + AI-destekli Playwright + testing-loop (maks 6 tekrar, düzelmezse raporla) + autonomous QA ajanı",
  },
  owasp: {
    tr: "OWASP & Standartlar",
    kapsa:
      "OWASP Top 10:2025 ilgili maddeleri (+ AI yüzeyi varsa LLM Top 10: prompt injection, güvensiz çıktı), bu görevin tehdit yüzeyi",
  },
  integration: {
    tr: "Kernel/Core Entegrasyonu",
    kapsa:
      "kernel/core/modüllerle entegrasyon: gerekli mi, hangi sözleşme/olay üzerinden, bağımlılık yönü",
  },
  moduleUsage: {
    tr: "Modül Kullanımı",
    kapsa:
