...
const DOM = {
  finance: {
    ent: ["fatura", "ledger kaydı", "tahsilat", "mizan", "KDV beyanı", "abonelik"],
    pii: "mali kayıt/IBAN",
    reg: "GİB e-fatura + KVKK",
    perf: "büyük defter sorgusu",
    evt: "fatura kesildi",
  },
  "core-operations": {
    ent: ["lead", "müşteri kartı", "sipariş", "stok hareketi", "POS satışı"],
    pii: "müşteri PII",
    reg: "KVKK",
    perf: "sipariş listesi",
    evt: "sipariş onaylandı",
  },
  hr: {
    ent: ["özlük kaydı", "bordro", "izin talebi", "performans formu"],
    pii: "özlük/SGK verisi",
    reg: "SGK + KVKK",
    perf: "bordro hesaplama",
    evt: "bordro kapandı",
  },
  "supply-chain": {
    ent: ["depo lokasyonu", "sevkiyat", "stok sayımı", "taşıma rotası"],
    pii: "taşıyıcı bilgisi",
    reg: "KVKK",
    perf: "stok hareketi sorgusu",
    evt: "sevkiyat çıktı",
  },
  "customer-revenue": {
    ent: ["destek talebi", "kampanya", "CPQ teklifi", "sadakat puanı"],
    pii: "müşteri iletişim verisi",
    reg: "KVKK + KVKK izin",
    perf: "talep kuyruğu",
    evt: "talep açıldı",
  },
  "content-collaboration": {
    ent: ["belge", "klasör", "wiki sayfası", "yorum"],
    pii: "paylaşım izinleri",
    reg: "KVKK",
    perf: "belge arama",
    evt: "belge paylaşıldı",
  },
  "data-intelligence": {
    ent: ["model", "embedding", "RAG kaynağı", "tahmin çıktısı"],
    pii: "eğitim verisi PII",
    reg: "AI + KVKK",
    perf: "vektör arama",
    evt: "tahmin üretildi",
  },
  kernel: {
    ent: ["ArcheType", "surface", "tenant context", "capability"],
    pii: "tenant verisi",
    reg: "tenant izolasyonu",
    perf: "metadata çözümleme",
    evt: "şema değişti",
  },
  scale: {
    ent: ["outbox mesajı", "saga adımı", "idempotency anahtarı", "realtime kanalı"],
    pii: "mesaj yükü",
    reg: "teslim garantisi",
    perf: "mesaj işleme hızı",
    evt: "mesaj yayınlandı",
  },
  layer1: {
    ent: ["workflow durumu", "audit olayı", "dosya nesnesi", "import işi"],
    pii: "olay aktörü",
    reg: "denetim izi",
    perf: "olay akışı",
    evt: "durum değişti",
  ...
  crosscut: {
    ent: ["observability sinyali", "KVKK kaydı", "kimlik", "i18n anahtarı"],
    pii: "kişisel veri",
    reg: "KVKK + gözlemlenebilirlik",
    perf: "log/iz akışı",
    evt: "ihlal tespit edildi",
  },
  sus: {
    ent: ["tipli aksiyon", "declarative türetme", "durable adım", "conformance testi"],
    pii: "aksiyon bağlamı",
    reg: "aksiyon yetki tavanı",
    perf: "aksiyon yürütme",
    evt: "aksiyon tetiklendi",
  },
  "platform-horizontal": {
    ent: ["Studio tanımı", "BPM süreci", "IAM rolü", "audit kaydı"],
    pii: "kullanıcı kimliği",
    reg: "erişim yönetimi",
    perf: "süreç yürütme",
    evt: "süreç başladı",
  },
  vertical: {
    ent: ["sektör kaydı", "mevzuat alanı", "özel akış"],
    pii: "sektörel kişisel veri",
    reg: "sektör mevzuatı",
    perf: "sektör sorgusu",
    evt: "kayıt güncellendi",
  },
  frontend: {
    ent: ["UI bileşeni", "tema token", "rota"],
    pii: "istemci durumu",
    reg: "CSP/XSS",
    perf: "ilk yük",
    evt: "rota değişti",
  ...
  backend: {
    ent: ["API uç noktası", "servis", "kuyruk"],
    pii: "istek verisi",
    reg: "API güvenliği",
    perf: "p95 gecikme",
    evt: "istek alındı",
  ...
  build: {
    ent: ["CI işi", "artefakt", "SBOM", "deploy adımı"],
    pii: "yapı sırrı",
    reg: "tedarik zinciri (SLSA)",
    perf: "derleme süresi",
    evt: "build tamamlandı",
  },
  dx: {
    ent: ["CLI komutu", "SDK çağrısı", "scaffold"],
    pii: "yerel gizli anahtar",
    reg: "geliştirici gizliliği",
    perf: "komut yanıtı",
    evt: "komut çalıştı",
  },
  layer0: {
    ent: ["atomik tip", "değer nesnesi", "şema primitifi"],
    pii: "—",
    reg: "tip güvenliği",
    perf: "doğrulama maliyeti",
    evt: "tip doğrulandı",
  },
};
const DEFAULT_DOM = {
  ent: ["kayıt", "varlık", "işlem"],
  pii: "kişisel veri",
  reg: "KVKK",
  perf: "liste sorgusu",
  evt: "durum değişti",
...
function itemsFor(n) {
  ...
  return {
    featureDefs: [
      `${T}: ${e1} ve ${e2} için net işlevsel sınır (${scope})`,
      `${e1} yaşam döngüsü + durum makinesi`,
      `Girdi/çıktı sözleşmesi ve hata yolları (${e2})`,
    ],
    security: [
      `${T}: ${e1} tablolarında tenant_id RLS + ${d.pii} maskeleme`,
      `${T} için rol bazlı erişim (least-privilege) + ${d.reg} uyum audit'i`,
      `${e2} işlemlerinde sunucu-taraflı doğrulama (${T})`,
    ],
    codeOptimization: [
      `${T} çekirdeğini UI'dan ayır; ${e1} tipleri katı (Zod)`,
      `${e2} yolunda ölü kod elemesi + kod bölme`,
      "Döngüsel karmaşıklık eşiği + lint kapısı",
    ],
    securityOptimization: [
      `${T}: ${e1} erişiminde secret rotasyonu + en az ayrıcalık`,
      `${T} için sıkı CSP + rate-limit`,
      `${d.reg} kapsamında bağımlılık taraması`,
    ],
    performance: [
      `${T}: ${d.perf} için bileşik indeks + imleç sayfalama`,
      `${e1} listesinde N+1 önleme + önbellek`,
      `${T} p95 gecikme hedefi ve ölçüm`,
    ],
    mobileApps: [
      `${T} ekranı 320px+ duyarlı; ${e1} kartı dokunma hedefi ≥44px`,
      "iOS/Android PWA (Capacitor köprüsü)",
      `${e2} için offline taslak + senkron`,
    ],
    wcag: [
      `${T} formlarında label↔input + ARIA hata mesajı`,
      `${e1} tablosunda klavye gezinme + kontrast ≥7:1`,
      "Görünür odak sırası + ekran okuyucu denetimi",
    ],
    deployment: [
      `${T} servisi Docker Swarm + sağlık kontrolü`,
      "Kubernetes: HPA + liveness/readiness probe + kaynak limiti",
      `Shared hosting: ${n.level === "app" || n.level === "module" ? "statik SPA + JSON" : "bağımsız artefakt"}`,
    ],
    eca: [
      `${T}: backend ruleset AI app/module üretim-güncelleme denemesini deny eder`,
      `${T} zincirinde döngü kırıcı (maks 6) + idempotency`,
      "ArcheType prod update: geçmiş veri korunur; append-only/expand-contract + step-up + rollback",
    ],
    aiAgents: [
      aiBoundary,
      "sub_prompt güvenilmez girdi; ruleset override/disable denemesi anında deny",
      `${d.reg} kapsamında PII redaksiyon + kill-switch`,
    ],
    testing: [
      `${T} için unit + e2e + '${e1}' kullanıcı yolculuğu (Playwright)`,
      "AI-destekli senaryo + testing-loop (maks 6, düzelmezse raporla)",
      `${e2} için autonomous QA + golden fixture`,
    ],
    owasp: [
      `${T} yüzeyinde OWASP Top 10:2025 (özellikle A01 erişim, A03 injection)`,
      `${e1} için ${n.source?.cluster === "data-intelligence" ? "LLM Top 10 (prompt injection)" : "girdi/çıktı doğrulama"}`,
      "Güvenlik olay loglaması + denetim izi",
    ],
    integration: [
      `${T}, kernel/core ile ${e1} sözleşmesi üzerinden entegre`,
      n.parentId
        ? "Üst düğümle tipli arayüz; yan modüllerle gevşek bağ"
        : "Çekirdek kontrol düzlemiyle doğrudan",
      `Olay veriyolu: '${d.evt}' yayını`,
    ],
    moduleUsage:
      n.level === "app"
        ? [`${T} ürün ailesi; alt modülleri tek panelde sunar`]
        : [
...
for (const f of files) {
  ...
  n.acceptanceCriteria = [
    `${n.title} için ${e} işlevsel kabulü karşılandı`,
    `${d.reg} + AAA erişilebilirlik kapıları geçti`,
  ];
  n.risks = [
    {
      id: `r-${n.id}-1`,
      desc: `${e} sözleşmesi/şeması üst katmanla uyumsuzlaşabilir`,
      severity: "medium",
      mitigation: "Sözleşme testleri + sürümleme (expand-contract)",
    },
    {
      id: `r-${n.id}-2`,
      desc: `${d.perf} ölçek altında yavaşlayabilir`,
      severity: "low",
