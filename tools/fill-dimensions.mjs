...
const CP = {
  kernel: {
    d: "metadata-driven ArcheType motoru ve çekirdek kontrol düzlemi",
    sec: "kernel düzeyinde yetki (authz) ve tenant izolasyonu",
    intg: "tüm üst katmanların dayandığı çekirdek",
  },
  scale: {
    d: "ölçek primitifleri (outbox, saga, idempotency, realtime)",
    sec: "mesaj bütünlüğü ve tekrar-saldırı koruması",
    intg: "kernel üstünde, modüllerin dayanıklılık tabanı",
  },
  layer1: {
    d: "in-tree yatay modül (workflow, audit, file, import)",
    sec: "modül sınırında yetki ve denetim izi",
    intg: "kernel+scale üstünde, app'lerce paylaşılan",
  },
  crosscut: {
    d: "kesişen ilgi (observability, privacy/KVKK, identity, i18n)",
    sec: "yatay güvenlik ve gizlilik kontrolleri",
    intg: "tüm app'lere enjekte edilen yatay servis",
  },
  cc: { d: "kesişen ilgi", sec: "yatay güvenlik/gizlilik", intg: "tüm app'lere yatay" },
  sus: {
    d: "platform yeteneği (tipli aksiyon yüzeyi, declarative türetme, durable execution)",
    sec: "aksiyon yüzeyi yetki tavanı ve sözleşme doğrulaması",
    intg: "kernel+layer1 üstünde otomasyon tabanı",
  },
  "platform-horizontal": {
    d: "yatay platform servisi (Studio, BPM, iPaaS, IAM, Audit)",
    sec: "kimlik/erişim yönetimi ve denetim",
    intg: "tüm app'lerin kullandığı yatay platform",
  },
  "core-operations": {
    d: "çekirdek operasyon ürünü (CRM, satış, e-ticaret, stok)",
    sec: "ticari veri RLS ve rol bazlı erişim",
    intg: "finans/tedarik modülleriyle entegre",
  },
  finance: {
    d: "finans & muhasebe (GİB e-fatura, çift kayıt, RevRec)",
    sec: "mali veri maskeleme, değişmez denetim izi, KVKK",
    intg: "ledger/billing çekirdeğiyle entegre",
  },
  "supply-chain": {
    d: "tedarik zinciri & lojistik (WMS, TMS, demand)",
    sec: "depo/sevkiyat yetkilendirmesi",
    intg: "stok ve satın alma modülleriyle",
  },
  hr: {
    d: "insan kaynakları (HRMS, bordro-TR, ATS)",
    sec: "özlük verisi gizliliği ve KVKK",
    intg: "muhasebe ve kimlik modülleriyle",
  },
  "customer-revenue": {
    d: "müşteri & gelir (helpdesk, pazarlama, CPQ)",
    sec: "müşteri PII koruması",
    intg: "CRM ve faturalama ile",
  },
  "content-collaboration": {
    d: "içerik & işbirliği (CMS, drive, wiki)",
    sec: "belge erişim kontrolü ve paylaşım",
    intg: "dosya ve kimlik modülleriyle",
  },
  "data-intelligence": {
    d: "veri & zeka (AI-first: copilot, RAG, BI)",
    sec: "PII redaksiyonu ve model girdisi denetimi",
    intg: "tüm modüllerin veri/AI tabanı",
  },
  vertical: {
    d: "dikey/sektörel çözüm (emlak, restoran, sağlık)",
    sec: "sektör mevzuatı uyumu",
    intg: "yatay modülleri sektöre uyarlar",
  },
  backend: { d: "backend altyapısı", sec: "API güvenliği", intg: "kernel ile" },
  frontend: {
    d: "frontend altyapısı (UI, tema, deploy)",
    sec: "XSS/CSP istemci koruması",
    intg: "tüm app UI'larının tabanı",
  },
  ...
  dx: {
    d: "geliştirici deneyimi (CLI, SDK)",
    sec: "yerel gizli anahtar yönetimi",
    intg: "tüm geliştirme akışı",
  },
  edu: { d: "eğitim içeriği", sec: "—", intg: "dokümantasyon" },
  kararlar: { d: "mimari karar (ADR)", sec: "karar gerekçesi", intg: "tüm mimariyi yönlendirir" },
  aday: {
    d: "aday ürün/özellik",
    sec: "fizibilite aşaması güvenlik taslağı",
    intg: "olgunlaşınca ilgili katmana",
  },
};
const DEFAULT_CP = {
  d: "modül",
  sec: "tenant izolasyonu ve rol bazlı erişim",
  intg: "kernel/core ile",
};
...
function dims(node) {
  ...
  const items = {
    ...
    integration: [
      `${cp.intg}`,
      node.parentId
        ? "Üst düğümle sözleşmeli arayüz; yan modüllerle gevşek bağ"
  ...
  const out = {};
  for (const k of DIMS)
    out[k] = { key: k, title: DIM_TR[k], status: "filled", items: items[k], notes: "" };
  return out;
}
...
function phases(node) {
  ...
  const st = {
    ...
    "release-maintenance": "pending",
  };
  const out = {};
  for (const p of PHASES)
    out[p] = { status: st[p], criteria: crit[p], passed: st[p] === "passed", notes: "" };
  return out;
}
...
for (const f of files) {
  ...
  if (!n.acceptanceCriteria?.length)
    n.acceptanceCriteria = [
      "İşlevsel kabul kriterleri karşılandı",
      "AAA erişilebilirlik + güvenlik kapıları geçti",
    ];
  if (!n.deliverables?.length)
    n.deliverables = [`${n.title} uygulaması`, "Testler + dokümantasyon"];
  if (!n.risks?.length)
    n.risks = [
      {
        id: `r-${n.id}-1`,
        desc: "Üst katman sözleşmesi değişebilir",
        severity: "medium",
        mitigation: "Sözleşme testleri + sürümleme",
      },
    ];
  if (!n.effort || !n.effort.estimate)
    n.effort = { estimate: EFFORT[n.level] || 5, unit: "sp", spent: 0 };
...
console.log(
  `[fill-dimensions] dolduruldu ${filled}, korundu (bespoke) ${skipped}, toplam ${files.length}.`,
);
