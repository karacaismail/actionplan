...
export const xdim = (P, what) => ({
  featureDefs: [
    `${P}: ${what}`,
    `${P} üst başlığın alt-detayı; tek sorumluluk`,
    `${P} örnek dal — granülerlikteki yerini gösterir`,
  ],
  security: [
    `${P} üst kümenin tenant izolasyonuna uyar`,
    `${P} hassas veri sınırda korunur`,
    `${P} kişisel veri üst katmanda maskelenir`,
  ],
  codeOptimization: [
  ...
  integration: [
    `${P} üst başlığa tipli arayüzle bağlanır`,
...
export function apply(label, CONTENT) {
  ...
  for (const [id, dims] of Object.entries(CONTENT)) {
    ...
    if (!fs.existsSync(fp)) {
      console.warn(`[${label}] atlandı (dosya yok): ${id}`);
      skipped++;
      continue;
    }
  ...
  console.log(
    `[${label}] ${applied} düğüm derinleştirildi (swarm)${skipped ? `, ${skipped} atlandı` : ""}.`,
  );
}
