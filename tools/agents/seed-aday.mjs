...
const CONTENT = {
  ...
  "dist-agritech": dim(
    ...
    ["Sensör verisi yüksek-hacim time-series; parsel sorgusu coğrafi indeks",
     "Hasat/tedarik raporu projeksiyondan", "Saha çevrimdışı + senkron"],
    ["Saha cihazları edge; merkez bulutta toplar (Kubernetes)",
     "IoT ağ geçidi ayrı; kötü bağlantıda kuyruk", "Shared hosting'de temel kayıt (IoT kısıtlı)"],
  ),
  ...
  "dist-clinic": dim(
    ...
    ["Hasta listesi/randevu indeksli; tıbbi geçmiş sayfalı",
     "Randevu çakışma kontrolü anlık", "Mobil hekim erişimi düşük gecikme"],
    ["Sağlık verisi izole; veri ikamet (residency) kuralına uyar",
     "Yedekleme + DR kritik (sağlık sürekliliği)", "Shared hosting sağlık için önerilmez (degrade)"],
  ),
  ...
  "dist-construction": dim(
    ...
    ["Hakediş hesabı saf; proje ilerleme rollup'lı",
     "Saha verisi (foto/konum) ayrı depo", "Mobil saha girişi çevrimdışı"],
    ["Saha mobil + merkez bulut; kötü bağlantıda offline kuyruk",
     "Standart yatay ölçek: durum-bilgisiz servis kopyaları", "Shared hosting'de temel proje takibi"],
  ),
  ...
  "dist-education": dim(
    ...
    ["Öğrenci/sınıf listesi indeksli; not girişi toplu",
     "Veli portalı okuma-yoğun önbellekli", "Mobil veli erişimi hafif"],
    ["Standart yatay ölçek; veli portalı CDN'li",
     "Reşit-olmayan veri ikamet kuralı", "Shared hosting'de küçük okul (uyumlu)"],
  ),
  ...
  "dist-legal": dim(
    ...
    ["Dosya araması full-text; süre takvimi indeksli",
     "Belge önizleme önbellekli", "Mobil duruşma takvimi erişimi"],
    ["Belge deposu şifreli + DR; gizlilik kritik",
     "Standart yatay ölçek: kuyruk derinliğine göre worker", "Shared hosting'de küçük büro (belge kısıtlı)"],
  ),
  ...
  "dist-membership": dim(
    ...
    ["Üye listesi indeksli; aidat durumu projeksiyondan",
     "Etkinlik kaydı eşzamanlı", "Mobil üye self-servis"],
    ["Standart yatay ölçek; etkinlik zirvesinde kuyruk",
     "KVKK veri yönetimi", "Shared hosting'de küçük dernek (uyumlu)"],
  ),
  ...
  "dist-ngo": dim(
    ...
    ["Bağış/gönüllü listesi indeksli; fon raporu projeksiyondan",
     "Kampanya zirvesi ölçeklenir", "Mobil bağış akışı hızlı"],
    ["Standart yatay ölçek; kampanya zirvesinde HPA",
     "Bağış verisi şeffaf+korumalı", "Shared hosting'de küçük STK (uyumlu)"],
  ),
  ...
  "dist-realestate": dim(
    ...
    ["İlan araması coğrafi+facet indeksli; portföy listesi hızlı",
     "Eşleştirme skoru anlık", "Mobil saha (ilan/foto) çevrimdışı"],
    ["İlan medyası nesne deposu+CDN; coğrafi sorgu PostGIS",
     "Standart yatay ölçek: okuma-replikası + önbellek", "Shared hosting'de temel portföy/ilan"],
  ),
  ...
  "dist-restaurant": dim(
    ...
    ["Adisyon işlemi düşük gecikme; mutfak ekranı gerçek-zaman",
     "Yoğun saatte sipariş kuyruğu", "Kasiyer çevrimdışı + senkron"],
    ["Mağaza-içi edge + merkez bulut; mutfak ekranı realtime",
     "Standart yatay ölçek: bölge-yerel dağıtım", "Shared hosting'de tek-şube (kısıtlı)"],
  ),
  ...
  "dist-sahibinden": dim(
    ...
    ["İlan araması facet+coğrafi indeksli; yüksek okuma önbellekli",
     "Mesajlaşma gerçek-zaman", "Mobil ilan verme/foto çevrimdışı"],
    ["Arama servisi ayrı ölçek; medya CDN; kampanya zirvesinde HPA",
     "Trust/fraud skorlama ayrı", "Shared hosting'de temel ilan (kısıtlı)"],
  ),
  ...
  "dist-site": dim(
    ...
    ["Aidat durumu projeksiyondan; arıza listesi indeksli",
     "Duyuru toplu bildirim", "Mobil sakin self-servis"],
    ["Standart yatay ölçek; küçük-orta site hacmi",
     "Aidat tahsilat ödeme entegrasyonu", "Shared hosting'de uyumlu"],
  ),
  ...
  "dist-travel": dim(
    ...
    ["Rezervasyon arama tedarikçi-agregasyonlu; müsaitlik anlık",
     "Voucher üretimi asenkron", "Mobil rezervasyon akışı"],
    ["Tedarikçi entegrasyonu (GDS) ayrı; rezervasyon merkezi",
     "Sezon zirvesinde HPA", "Shared hosting'de küçük acente (kısıtlı)"],
  ),
  ...
  "dist-veteriner": dim(
    ...
    ["Hayvan/sahip listesi indeksli; aşı takvimi sorgusu hızlı",
     "Randevu çakışma anlık", "Mobil saha/ev ziyareti çevrimdışı"],
    ["Standart yatay ölçek; klinik hacmi",
     "Yedekleme (tıbbi süreklilik)", "Shared hosting'de küçük klinik (uyumlu)"],
  ),
  ...
  "edition-creator": dim(
    ...
    ["İçerik teslimi CDN; abone erişim kontrolü hızlı",
     "Üyelik katmanı kontrolü O(1)", "Mobil içerik tüketimi öncelikli"],
    ["İçerik CDN + medya depo; standart ölçek",
     "Ödeme/abonelik entegrasyonu", "Shared hosting'de temel içerik (medya kısıtlı)"],
  ),
  ...
  "edition-departman-copilot": dim(
    ...
    ["RAG yanıtı düşük gecikme; yetki filtresi arama-seviyesinde",
     "Embedding önbellekli", "Mobil copilot akış yanıt"],
    ["Çıkarım AI Stack üzerinden ayrı ölçek; departman verisi izole",
     "Maliyet bütçe tavanı", "Shared hosting AI için kısıtlı (degrade)"],
  ),
  ...
  "edition-onmuhasebe": dim(
    ...
    ["Cari/kasa sorgusu indeksli; rapor basit projeksiyon",
     "e-Fatura gönderim kuyruğu", "Mobil fatura/tahsilat"],
    ["Standart yatay ölçek; GİB gönderici worker",
     "TR veri uyumu", "Shared hosting'de KOBİ için uyumlu"],
  ),
  ...
  "edition-people": dim(
    ...
    ["Bordro hesabı toplu/asenkron; özlük sorgusu indeksli",
     "İzin bakiyesi anlık", "Mobil çalışan self-servis (izin/bordro)"],
    ["Standart yatay ölçek; bordro dönem-sonu worker",
     "SGK/KVKK veri uyumu", "Shared hosting'de küçük şirket (uyumlu)"],
  ),
  ...
  "edition-randevu": dim(
    ...
    ["Takvim/müsaitlik sorgusu anlık; çakışma kontrolü O(1)",
     "Hatırlatma kuyruğu", "Mobil online randevu öncelikli"],
    ["Standart yatay ölçek; hatırlatma worker",
     "Online ödeme entegrasyonu", "Shared hosting'de uyumlu"],
  ),
  ...
  "edition-salescrm": dim(
    ...
    ["Sales CRM Edition satış ekiplerine varyant sunar",
     "Satış app'leri bu edition'ı kullanır"],
    ["Pipeline sorgusu indeksli; aktivite akışı sayfalı",
     "Skorlama anlık", "Mobil saha satış (offline aktivite)"],
    ["Standart yatay ölçek; raporlama okuma-replikası",
     "E-posta/takvim entegrasyonu", "Shared hosting'de uyumlu"],
  ),
  ...
  "edition-storefront": dim(
    ...
    ["Vitrin/katalog CDN+önbellek; sepet düşük gecikme",
     "Kampanya zirvesinde HPA", "Mobil-öncelikli vitrin/ödeme"],
    ["Vitrin edge/CDN; sipariş merkezi; kampanya HPA",
     "Ödeme entegrasyonu", "Shared hosting'de statik vitrin + JSON sipariş"],
  ),
  ...
  "s-channel-hub": dim(
    ...
    ["Kanal senkronu olay-temelli artımlı; oran-sınırına uyum",
     "Sipariş webhook asenkron", "Çoklu kanal paralel eşitleme"],
    ["Kanal entegrasyonu ayrı worker (her kanal oran-sınırı)",
     "Standart yatay ölçek: durum-bilgisiz servis kopyaları", "Shared hosting'de az-kanal (kısıtlı)"],
  ),
  ...
  "s-comms": dim(
    ...
    ["Mesaj gerçek-zaman düşük gecikme; medya ayrı işleyici",
     "Çok-katılımcı toplantı ölçeği", "Mobil görüşme/mesaj öncelikli"],
    ["Medya/sinyal sunucusu ayrı ölçek (WebRTC); mesaj realtime",
     "Eşzamanlı toplantı için HPA", "Shared hosting görüşme için kısıtlı"],
  ),
  ...
  "s-esign": dim(
    ...
    ["İmza akışı durum makinesi; belge hash doğrulama hızlı",
     "Çoklu imzacı paralel/sıralı", "Mobil imza (parmak/çizim) erişimli"],
    ["İmza servisi standart; belge şifreli depo + DR",
     "Yasal saklama süresi", "Shared hosting'de temel imza (kısıtlı)"],
  ),
  ...
  "s-iot": dim(
    ...
    ["Telemetri yüksek-hacim akış; zaman-serisi deposu",
     "Komut düşük gecikme", "Mobil cihaz izleme/komut"],
    ["MQTT broker + ingest ayrı ölçek; cihaz edge",
     "Çok-bölge cihaz yönetimi", "Shared hosting IoT için kısıtlı (degrade)"],
  ),
  ...
  "s-isg": dim(
    ...
    ["Risk/olay listesi indeksli; eğitim takvimi sorgusu",
     "Raporlama projeksiyondan", "Mobil saha olay bildirimi (foto)"],
    ["Standart yatay ölçek; yasal raporlama worker",
     "Yasal saklama + audit", "Shared hosting'de temel İSG (uyumlu)"],
  ),
  ...
  "s-kvkk": dim(
    ...
    ["Envanter sorgusu indeksli; talep takibi projeksiyondan",
     "Silme/export arka plan işi", "Mobil rıza/talep self-servis"],
    ["Standart yatay ölçek; TR veri ikameti",
     "Yasal saklama + ihlal süre takibi", "Shared hosting'de temel KVKK (uyumlu)"],
  ),
  ...
  "s-mail": dim(
    ...
    ["Toplu gönderim parçalı kuyruk; oran-sınırına uyum",
     "Teslim takibi asenkron", "Mobilde kampanya durumu"],
    ["Gönderim worker'ı ayrı; SMTP/sağlayıcı adaptörü",
     "IP ısıtma/itibar yönetimi", "Shared hosting'de temel e-posta (kısıtlı)"],
  ),
  ...
  "s-scheduling": dim(
    ...
    ["Müsaitlik sorgusu anlık; çakışma kontrolü eşzamanlı-güvenli",
     "Hatırlatma kuyruğu", "Mobil takvim erişimi"],
    ["Standart yatay ölçek; hatırlatma worker",
     "Takvim senkron (CalDAV opsiyon)", "Shared hosting'de uyumlu"],
  ),
  ...
  "stack-builder": dim(
    ...
    ["Tasarım önizleme artımlı; şema doğrulama hızlı",
     "Diff hesabı anlık", "Builder masaüstü öncelikli (mobil önizleme)"],
    ["Builder kernel'e yazar; ortam ayrımı (draft/prod)",
     "Standart yatay ölçek: kuyruk derinliğine göre worker", "Shared hosting'de tanım dışa-aktarımı"],
  ),
  ...
  "stack-channel": dim(
    ...
    ["Kanal senkronu artımlı; oran-sınırlı",
     "Sipariş asenkron toplama", "Mobilde kanal durumu"],
    ["Kanal entegrasyon worker'ları; her kanal oran-sınırı",
     "Standart yatay ölçek: okuma-replikası + önbellek", "Shared hosting'de az-kanal (kısıtlı)"],
  ),
  ...
  "stack-compliance": dim(
    ...
    ["Uyum durumu projeksiyondan; e-belge gönderim kuyruğu",
     "Talep takibi indeksli", "Mobilde uyum/talep durumu"],
    ["GİB/e-imza gönderici worker; TR veri ikameti",
     "Standart yatay ölçek: bölge-yerel dağıtım", "Shared hosting'de temel uyum (uyumlu)"],
  ),
  ...
  "stack-editions": dim(
    ...
    ["Edition çözümleme bayrak okuma O(1)",
     "Varyant geçişi anlık", "Mobilde aktif edition göstergesi"],
    ["Edition yapılandırması imaja/konfige gömülü; tutarlı",
     "Standart yatay ölçek: durum-bilgisiz servis kopyaları", "Shared hosting'de sınırlı edition seti"],
  ),
  ...
  "stack-messaging": dim(
    ...
    ["Sohbet yanıtı düşük gecikme; webhook asenkron",
     "Eşzamanlı sohbet ölçeği", "Mobil-doğal (mesajlaşma zaten mobil)"],
    ["Kanal webhook alıcısı ayrı; çıkarım AI Stack'ten",
     "Standart yatay ölçek: kuyruk derinliğine göre worker", "Shared hosting'de temel (kısıtlı)"],
  ),
  ...
  "stack-service": dim(
    ...
    ["İş emri/atama indeksli; rota optimizasyon asenkron",
     "Saha senkron", "Mobil saha (offline iş emri) öncelikli"],
    ["Saha mobil + merkez bulut; rota servisi ayrı",
     "Standart yatay ölçek: okuma-replikası + önbellek", "Shared hosting'de temel iş emri"],
  ),
  ...
  "stack-workspace": dim(
    ...
    ["Belge araması full-text/RAG; sürüm geçmişi sayfalı",
     "Eşzamanlı düzenleme (CRDT opsiyon)", "Mobil belge erişimi"],
    ["Belge nesne deposu + CDN; arama ayrı servis",
     "Standart yatay ölçek + DR", "Shared hosting'de temel belge (kısıtlı)"],
  ),
};
...
console.log(`[seed-aday] ${applied} aday düğümü derinleştirildi (swarm)${skipped ? `, ${skipped} atlandı` : ""}.`);
