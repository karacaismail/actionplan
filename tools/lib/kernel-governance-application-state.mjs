...
const ATTESTATIONS = { "KGA-D01": { ref: "reports/kernel-code-bearing-descendant-handoff-2026-07-15.json", scope: "approved-descendant-materialization", summary: { approved: 33, applied: 33, remaining: 0 } }, "KGA-D02": { ref: "reports/kernel-surface-dependency-order-handoff-2026-08-01.json", scope: "governance-semantics-record", summary: { approved: 1, applied: 1, remaining: 0, applicationScope: "governance-semantics-record", canonicalEdgeApplication: "deferred-to-KGA-D07" } } };
...
const attests = (artifact, id, contract) => Boolean(artifact) && artifact.decisionId === id && artifact.status === "approved-application-pending" && artifact.gapClosed === false && same(artifact.applicationSummary, contract.summary) && artifact.provenance?.approval?.normalizedSelectionSha256 === SELECTION_SHA256;
