...
Kanıt disiplini makineye `tools/agents/check-execution-readiness.mjs` (BLOKLAYICI; `deploy.yml` içinde) ile bağlanır. Bu kapı `src/data/generated/nodes` altındaki doğrulanmış current-live düğüm evrenini tarar ve üç kuralı zorlar; ilki doğrudan kanıt zorlamasıdır:
