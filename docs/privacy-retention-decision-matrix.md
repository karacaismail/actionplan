...
- Global pazar ekseni (§17): G1–G12 eksenlerinin her biri hedef pazar için ayrı madde olarak ele alınıyor; veri bölgesi seçimi ve sınır ötesi aktarım envanteri kayıtlı; veri merkezinin Avrupa'da olması tek başına uyumluluk sayılmıyor (aktarım araçları, alt işleyen zinciri, destek erişimleri, yedekler ve DR trafiği birlikte değerlendiriliyor).
- Hukuki içerik sürümleme (§17): her sözleşme ve gizlilik politikası sürümlü; kullanıcının hangi dilde hangi sürümü ne zaman kabul ettiği kayıtlı; bağlayıcı dil ile gösterim çevirisi ayrık; hukuki metinler normal ürün çevirisi iş akışına bırakılmıyor; acil politika güncellemesi tüm pazarları kapsayan mekanizmayla yayılıyor.
- Sahiplik sınırı çift-tanımsız: destek/operasyon dili taahhütleri ve yaptırım/coğrafya ürün politikası `global-market-readiness-directive.md`'de, analitik consent-kapsam farkları `decision-grade-data-contract.md`'de yaşıyor; bu doküman yalnız çapraz referans veriyor.
...
- Global pazar ekseni (§17): G1–G12 değerlendirmesi ve hukuki içerik sürümleme kuralları §13 AC'leriyle karşılandı; PRM-20–PRM-27 gereksinimleri test-kanıtlı; sahiplik sınırı (`global-market-readiness-directive.md`, `decision-grade-data-contract.md`) çift-tanımsız.

## 16. Compliance karşılığı (GDPR / KVKK / VUK / TTK / eIDAS)

Aşağıdaki tablo, bu karar sözleşmesinin gizlilik/saklama/denetim çerçevelerini matris ve çatışma-çözüm öğelerine nasıl eşlediğini gösterir; her satır bir uyum yükümlülüğünü bu dokümanın normatif kararına bağlar. Çerçeveler: GDPR (AB veri koruma, silme/unutulma hakkı Art.17), KVKK (Türkiye kişisel verilerin korunması m.7 silme/yok etme), VUK/TTK (Türkiye vergi/ticaret hukuku saklama zorunluluğu), eIDAS (AB delil/imza saklama). §17 global pazar ekseni bu tabloya sınır ötesi aktarım (GDPR Bölüm V / KVKK m.9), çerez ve izleme izinleri, ebeveyn izni, tüketici/cayma ve sözleşme-sürümleme yükümlülüklerini ekler.
...
| GDPR Bölüm V (Art.44–49) / KVKK m.9: sınır ötesi aktarım, EEA-dışı özel mekanizma | §17 G2–G3 + normatif uyarı (veri merkezi konumu tek başına uyumluluk değildir; aktarım araçları, alt işleyen zinciri, destek erişimleri, yedekler, DR trafiği birlikte değerlendirilir) |
| ePrivacy/çerez ve izleme izinleri; GDPR Art.8 yaş eşiği ve ebeveyn izni; yerel tüketici mevzuatı (cayma, otomatik yenileme) | §17 G7–G10 (pazar-bazlı izin/cayma rejimi; kayıtlar denetlenebilir) |
| Sözleşme/DPA yönetişimi: kabul kanıtı, bağlayıcı dil, sürüm izlenebilirliği | §17 hukuki içerik sürümleme (kabul kaydı: kullanıcı+dil+sürüm+zaman; sözleşme/DPA/alt işleyen listesi tutarlı sürümlenir) |

## 17. Global pazar hukuku, veri yerleşimi ve sözleşme yönetişimi (normatif)

**Nedir/yapar/yapmaz:** i18n bir metni çevirebilir; o metnin hedef pazarda **hukuken geçerli olup olmadığını belirleyemez** — çeviri bir dil işidir, hukuki geçerlilik ayrı bir uyum eksenidir. Bu bölüm, global SaaS için çeviriden bağımsız ele alınması zorunlu hukuk/yerleşim/sözleşme eksenlerini (G1–G12) ve hukuki içerik sürümleme kurallarını sabitler; §6 veri-sınıfı × muamele matrisiyle **çelişmez, onu tamamlar** (matris "bu veriye hangi muamele izinli?" sorusunu, bu bölüm "bu veri hangi hukuki dayanakla, hangi bölgede, hangi sözleşme sürümü altında işleniyor?" sorusunu çözer). Yeni bir kilit/imha motoru yazmaz, yerelleştirme altyapısı kurmaz, pazar-bazlı hukuki görüşü kendisi üretmez. Aktör iş bölümü değişmez (§10): ajan boşluk/çelişki *önerir* (draft), pazar-bazlı hukuki karar **yalnız insandır** (hukuk/uyum), motor onaylı kararı uygular; hiçbir hukuki dayanak, veri bölgesi veya sözleşme sürümü kararı AI'a devredilemez.

**Global hukuk-kapsam eksenleri (G1–G12):** Her hedef pazar için aşağıdaki on iki eksen çeviriden bağımsız ve her biri **ayrı madde** olarak ele alınır; biri diğerinin içinde eritilemez, "çeviri tamam" hiçbirini kapatmaz.

Bu tablo on iki global hukuk-kapsam eksenini (eksen → normatif beklenti ve bağ) tanımlar.

| # | Eksen | Normatif beklenti ve bağ |
|---|---|---|
| G1 | Hukuki dayanak | Kullanıcı verisinin hangi hukuki dayanakla işlendiği (sözleşme, açık rıza, meşru menfaat, yasal yükümlülük) her veri sınıfı ve hedef pazar için kayıtlıdır; §5 `legal_basis` alanı bu beyanın taşıyıcısıdır ve pazar-bazında farklılaşabilir. Dayanaksız işleme yoktur. |
| G2 | Veri yerleşimi ve veri bölgesi seçimi | Tenant verisinin hangi coğrafi bölgede durduğu (veri bölgesi) açık, denetlenebilir bir yapılandırma kaydıdır; "nerede duruyor" cevabı pazarlama beyanı değil kayıttır. Bölge seçimi §9 katman kuralına uyar (override yalnız daha katı yönde). |
| G3 | Sınır ötesi veri aktarımı | Verinin bölge dışına her çıkışı (replikasyon, destek erişimi, log/telemetri, yedek, DR) sınır ötesi aktarım sayılır ve aktarım mekanizması belgelenir; mekanizmasız bölge-dışı akış reddedilir (normatif uyarı bu bölümdedir). |
| G4 | Alt işleyenler ve üçüncü taraf servisler | Her alt işleyen (hosting, e-posta, analitik, LLM sağlayıcı) envanterde kayıtlıdır; alt işleyen değişikliği bildirime tabidir; LLM zero-retention bağı §7 K6 / `k-provider-adapter`'dadır. Envanter-dışı servis kullanımı yasaktır. |
| G5 | Saklama, silme, anonimleştirme, legal hold | Bu eksenin tamamı bu dokümanın çekirdeğidir: §6 matrisi, §7 K1–K6 kuralları ve `k-legal-hold-retention` mekaniği geçerlidir; burada **tekrar tanımlanmaz, referans verilir**. |
| G6 | Kullanıcı veri talepleri | Erişim, düzeltme, dışa aktarma ve silme talepleri tanımlı süreç ve süreyle işler; silme talebi §7 kurallarından geçer (K1 finansal iskelet, K2 hold, K4–K6 backup/vektör/LLM yüzeyleri). |
| G7 | Çerez ve izleme izinleri | Çerez ve benzeri izleme teknolojileri pazar-bazlı izin rejimine (opt-in/opt-out) bağlanır; çerez izni kaydı denetlenebilirdir; analitik consent-kapsam farkları `decision-grade-data-contract.md`'de yaşar. |
| G8 | Yaşa bağlı kullanım ve ebeveyn izni | Yaş eşiği pazara göre değişir; eşik altı kullanıcı için ebeveyn izni akışı ve bu verinin özel muamelesi tanımlanır; ebeveyn izni kaydı kabul-kanıtı gibi saklanır. |
| G9 | Yerel tüketici mevzuatı | Hedef pazarın tüketici koruma kuralları (ön bilgilendirme, fiyat gösterimi, haksız şart denetimi) sözleşme ve satış akışına yansıtılır; tüketici-tacir ayrımı sözleşme sürümünde açıktır. |
| G10 | Otomatik yenileme, iptal ve cayma | Abonelikte otomatik yenileme bildirimi, kolay iptal ve cayma hakkı süreçleri pazar kuralına bağlanır; cayma süresi ve istisnaları yürürlükteki sözleşme sürümünde beyan edilir. |
| G11 | Sektörel düzenlemeler | Finans, sağlık, eğitim ve kamu dikeyleri ek yükümlülük getirir (ek saklama, yerleşim, denetim, sertifikasyon); dikey gereksinim matris kararını **yalnız daha katı yönde** etkileyebilir (§9 ile hizalı). |
| G12 | İhracat kontrolleri, yaptırımlar, hizmet verilemeyen bölgeler | Yaptırım listeleri ve ambargo bölgeleri servis-verilebilirlik kontrolüne bağlanır; yaptırım taraması olmadan hedef pazara hizmet açılmaz; yaptırım/coğrafya ürün politikasının sahibi `global-market-readiness-directive.md`'dir. |

**Veri yerleşimi tek başına uyumluluk değildir (normatif uyarı):** Veri merkezinin Avrupa'da bulunması **tek başına uyumluluk değildir**; veri bölgesi seçimi gerekli ama yetersiz koşuldur. Birlikte değerlendirilmesi zorunlu yüzeyler: (a) **aktarım araçları** — verinin bölge dışına hangi hukuki mekanizmayla çıkabildiği; (b) **alt işleyen zinciri** — alt işleyenin (ve onun alt işleyeninin) verdiği bölge/aktarım taahhüdü; (c) **destek erişimleri** — bölge dışından yapılan yönetim/destek erişimi de bir aktarımdır; (d) **yedekler** — backup kopyasının durduğu bölge (§7 K4 crypto-shredding bağı); (e) **felaket kurtarma trafiği** — DR replikasyonunun geçtiği ve durduğu bölgeler. EEA dışına aktarımlar için özel mekanizma ve koşullar vardır; değerlendirme bu çerçeveye göre yapılır (kaynak: Avrupa Komisyonu/EDPB — https://commission.europa.eu/law/law-topic/data-protection/rules-business-and-organisations/obligations/what-rules-apply-if-my-organisation-transfers-data-outside-eu_en).

**Hukuki içerik sürümleme (normatif alt bölüm):** Hukuki içerik kod gibi sürümlenir; aşağıdaki kurallar bağlayıcıdır ve hiçbir pazar için gevşetilemez:

- **Sürüm zorunluluğu:** Her sözleşme ve gizlilik politikası sürümlenir; sürümsüz hukuki metin yayınlanamaz.
- **Kabul kaydı:** Kullanıcının **hangi dilde hangi sürümü ne zaman** kabul ettiği kayıt altına alınır (kullanıcı `PartyRef` + dil + sürüm + zaman); kabul kaydı append-only audit'e düşer ve sözleşme-kanıtı sınıfının §6 muamele kurallarına tabidir.
- **Bağlayıcı dil ayrımı:** Sözleşmenin bağlayıcı dili ile kullanıcıya gösterilen çeviri dili **ayrılır**; çeviri bilgilendiricidir, uyuşmazlıkta bağlayıcı dil esas alınır ve bu ayrım metnin kendisinde beyan edilir.
- **Çeviri iş akışı ayrımı:** hukuki metinler normal ürün çevirisi iş akışına bırakılmaz; hukuki metin çevirisi hukuk-onaylı ayrı bir süreçten geçer (ajan taslak önerebilir; yürürlük kararı insan/hukuktur, §10 ile hizalı).
- **Acil yayın mekanizması:** Acil politika güncellemeleri için **tüm pazarları ve dilleri kapsayan** yayın mekanizması bulunur; hangi pazarın hangi sürümde olduğu izlenebilirdir; hiçbir pazar eski sürümde unutulamaz.
- **Tutarlı sürümleme:** Sözleşme, DPA, alt işleyen listesi ve güvenlik dokümanları **tutarlı** (birbirine sürüm-referanslı) sürümlenir; bir DPA'nın işaret ettiği alt işleyen listesi sürümü belirsiz olamaz.

**Sahiplik sınırı (çapraz referans):** Bu bölüm iki kardeş sözleşmeyle sınır paylaşır ve çift tanım yasağına uyar: destek/operasyon dili taahhütleri ile yaptırım/coğrafya ürün politikası `global-market-readiness-directive.md`'de; analitik consent-kapsam farkları `decision-grade-data-contract.md`'de yaşar. Bu doküman o iki alanı tekrar tanımlamaz; yalnız veri-sınıfı × muamele matrisine dokunan yüzeylerini (G7 çerez-consent, G12 yaptırım/servis-verilebilirlik) bağlar.
...
| PRM-20 | G1–G12 global eksen listesi hedef pazar başına ayrı ele alınır; hukuki dayanak (G1) pazar-bazlı `legal_basis` beyanına bağlanır | Compliance | P1 | Integration | Dayanaksız işleme reddedilir; eksen değerlendirmesi kayıtlı | governance |
| PRM-21 | G2–G3: veri bölgesi seçimi denetlenebilir kayıt; sınır ötesi aktarım (destek erişimi, yedek, DR dahil) mekanizmasız yürümez | Compliance/Data | P0 | Integration | Mekanizmasız bölge-dışı aktarım reddedilir | governance |
| PRM-22 | G4: alt işleyen envanteri tam ve değişiklik-bildirimli; envanter-dışı üçüncü taraf servis kullanılmaz | Compliance | P1 | Integration | Envanter-dışı alt işleyen reddedilir | governance |
| PRM-23 | G6: kullanıcı veri talepleri (erişim/düzeltme/dışa aktarma/silme) tanımlı süreçte; silme K1–K6'dan geçer | Compliance | P1 | Integration | DSAR silmesi matris kurallarına bağlı | governance |
| PRM-24 | G7–G10: çerez/izleme izni, ebeveyn izni, tüketici mevzuatı, otomatik yenileme/iptal/cayma pazar-bazlı kurala bağlı ve kayıtlı | Compliance | P1 | Integration | İzin/cayma kayıtları denetlenebilir | governance |
| PRM-25 | Hukuki içerik sürümleme: kabul kaydı (kullanıcı+dil+sürüm+zaman), bağlayıcı dil ↔ gösterim çevirisi ayrımı, hukuki çeviri ayrı iş akışında | Compliance/Data | P0 | Integration | Sürümsüz/kayıtsız kabul reddedilir | governance |
| PRM-26 | Acil politika yayını tüm pazarları kapsar; sözleşme/DPA/alt işleyen listesi/güvenlik dokümanları tutarlı sürümlenir | Compliance | P1 | Integration | Pazar-sürüm eşlemesi izlenebilir | governance |
| PRM-27 | G11–G12: sektörel yükümlülük yalnız daha katı yönde etki eder; yaptırım/ambargo taraması servis-verilebilirlik kapısında | Compliance | P1 | Integration | Yasaklı bölgeye hizmet reddedilir | governance |

---

*Kaynak yönerge: Privacy / Audit / Bitemporal / Retention Decision Matrix (GDPR/KVKK/VUK/TTK/eIDAS). Bu doküman `k-legal-hold-retention-directive.md`'nin **karar-matrisi genişletmesidir** (çelişmez; muamele-sınıfı × veri-sınıfı ekseni ekler); K2 (hold kazanır) kuralı oradan referanslanır. Kardeş sözleşmeler: `k-legal-hold-retention-directive.md` (kilit/imha mekaniği), `core-contract-pack.md §2.5` (audit append-only — silinmez), `pdp-policy-contract.md` (muamele = policy-as-data, yetki kararı), `k-storage-dam-directive.md` (backup/binary silme, crypto-shredding), `k-provider-adapter-directive.md` (LLM zero-retention sözleşmesi), `atomik-netlestirme-2026-07-01.md` (EnumType/Duration/PartyRef atomları), `core-contract-pack.md §3.0.1` (AI guardrail), `global-market-readiness-directive.md` (destek/operasyon dili taahhütleri, yaptırım/coğrafya ürün politikası — §17 sahiplik sınırı), `decision-grade-data-contract.md` (analitik consent-kapsam farkları). Bu doküman hiçbir kod/şema/JSON dosyasına dokunmaz; yalnız sözleşme metnidir. Çelişki halinde `core-contract-pack.md` (kernel runtime) ve `k-legal-hold-retention` (kilit mekaniği) önceliklidir; bu doküman güncellenir. Sözleşmeyi değiştirme, veri silme, muamele-sınıfı atama ve çatışma-kuralı override yetkisi yalnız insan (hukuk/uyum/veri-yönetişim) onayındadır; AI bu dosyayı doğrudan güncelleyemez ve hiçbir veriyi silemez.*
