...
## 16. CI kapısı (workflow motoru için önerilen — henüz yürürlükte değil)

Bu bölüm yalnız **gerçek durumu** kaydeder: `tools/agents/check-state-machine-consistency.mjs` artık `qa:flow`/CI zincirindedir, fakat bu kapı `traceability.implementationStatus` ile `status`/`phase` arasındaki *WBS-düğüm* semantik kaymasını denetler (görev takibi amaçlı). `workflow_def`/`workflow_transition` gibi Workflow motorunun kendi durum-makinesi verisini **doğrulamaz**. Yani "check-state-machine CI'da çalışıyor ve Workflow motorunu koruyor" ifadesi bugün **doğru değildir** — workflow-motor-özel kapı henüz mevcut değildir.
