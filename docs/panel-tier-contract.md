...
**Storybook bağı:** her panel-tier'ın örnek ekranları elle çizilmiş mockup olarak değil, doğrudan **Master Component composition story'leri** olarak yaşar (`docs/storybook-implementation.md` §3 Surfaces katmanı; `docs/surface-spec.md` "Surface × Master Component / Storybook bağı"): developer/kernel-admin, tenant süper-admin ve son-kullanıcı panel örnekleri aynı Master Component kataloğunu farklı fixture/args ve rol-permission story'leriyle (editable/readonly/hidden-denied) kompoze eder — 18 ürün × 3 tier, 54 ayrı ekran seti değil, tek katalogdan türetilmiş composition story ailesidir.
