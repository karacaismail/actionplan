...
const successor = (chain: Chain) => chain.entries?.find((entry) => entry?.seq === 3);
...
describe("kernel runtime successor policy (EPOCH-03)", () => {
  it("appends EPOCH-03 over frozen predecessors and holds the full successor contract", async () => {
    ...
    // 1. Append-only shape: a third epoch, no sealed predecessor demoted.
    expect(chain.appendOnly).toBe(true);
    expect(chain.entries.map((entry) => entry.seq)).toEqual([1, 2, 3]);
    expect(head.epochId).toMatch(/03$/);
    ...
    expect(head.supersedes).toBe(2);
    expect(head.supersessionScope).toBe("KERNEL_RUNTIME_APPROVED_SHARDS_ONLY");
    ...
    expect(chain.chainHeadSeq).toBe(3);
    expect(chain.chainHeadEntrySha256).toBe(head.entrySha256);
    expect(chain.entries.map((entry) => entry.supersedes)).toEqual([null, 1, 2]);
    ...
    expect(terminal.map((entry) => entry.seq)).toEqual([3]);
    ...
    // 3. Exact EPOCH-03 effective tokens and scope.
    const tokens = tokenMap(head.normalizedText ?? "");
    for (const [token, value] of Object.entries(EPOCH03_TOKENS))
      expect(tokens.get(token), `EPOCH-03 token ${token}`).toBe(value);
    expect(tokens.get("SCOPE")).toBe(head.supersessionScope);
    for (const [key, cell] of Object.entries(head.dimensions ?? {}))
      expect(tokens.get(cell?.token), `EPOCH-03 dimension ${key}`).toBe(cell?.value);
    ...
    // 5. Deterministic reseal over three linked entries.
    for (const entry of chain.entries) expect(entryDigest(entry)).toBe(entry.entrySha256);
    expect(head.previousEntrySha256).toBe(EPOCH02_ENTRY_SHA256);
    ...
    // 6. Auth gate, platform prohibition and NO-GO floors stay effective under EPOCH-03.
    ...
    const matrix: Array<[string, (input: ReturnType<typeof fixture>) => void]> = [
      ...
      ["code-start", (input) => retext(input, "CODE_START=NO", "CODE_START=YES")],
      ["runtime-code", (input) => retext(input, "RUNTIME_CODE=NO", "RUNTIME_CODE=YES")],
      ["verdict", (input) => retext(input, "VERDICT=NO-GO", "VERDICT=GO")],
      ["kernel-runtime-writer", (input) => retext(input, "KERNEL_RUNTIME_WRITER=CLAUDE_ONLY_FAIL_CLOSED", "KERNEL_RUNTIME_WRITER=CODEX_ONLY")],
      ["scope", (input) => retext(input, "SCOPE=KERNEL_RUNTIME_APPROVED_SHARDS_ONLY", "SCOPE=ALL_TARGETS")],
      ["supersedes-token", (input) => retext(input, "SUPERSEDES=EPOCH-02", "SUPERSEDES=EPOCH-01")],
    ...
    // 8. Regression: a historical consumer stamp must not drift merely because chainHeadSeq
    //    advanced. The node universe resolves stamps against sealed entries, so the EPOCH-02
    //    handoff stays valid under an EPOCH-03 head — and a stamp naming no sealed entry
    //    still fails closed.
    // biome-ignore lint/suspicious/noExplicitAny: the pure JavaScript oracle has no declaration file.
