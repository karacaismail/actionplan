...
export function WorkloadView() {
  ...
  const option = useMemo(
    () => ({
      ...
      xAxis: { type: "category", data: loads.map((l) => nameOf(l.assignee)), axisLabel: { color: "#94a3b8", interval: 0, rotate: 30 } },
      ...
      series: [
        {
          ...
          barMaxWidth: 36,
          data: loads.map((l) => ({ value: Math.round(l.days * 10) / 10, itemStyle: { color: STATUS_COLOR[l.status] } })),
        },
        { type: "line", name: t.workload.capacity, data: loads.map((l) => l.capacity), itemStyle: { color: "#a78bfa" } },
      ],
    }),
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <Stat icon="ph-clock" label={t.workload.daysUnit} value={String(Math.round(loads.reduce((a, l) => a + l.days, 0)))} />
      </div>
          ...
          <tbody>
            {loads.map((l) => (
              <tr key={l.assignee} className="border-border border-b">
                ...
                <td className="px-3 py-2 text-right tabular-nums">{Math.round(l.days * 10) / 10}</td>
                <td className="px-3 py-2 text-right tabular-nums">{l.capacity}</td>
                <td className="px-3 py-2">
                  <span className="flex items-center gap-2">
                    <span className="size-2 rounded-full" style={{ background: STATUS_COLOR[l.status] }} aria-hidden="true" />
